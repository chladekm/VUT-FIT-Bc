...
**
** V pøípadì, že funkce dostane jako parametr q == NULL, volejte funkci
...
*/
	// Modulo - Pokud bude nasledujici index vetsi nez je velikost pole
	// zacne zase od začátku (pr. 52 % 50 = 2)
...
*/
	...
	return (q->b_index == q->f_index) ? 1 : 0; //Pokud je seznam prazdny, tak vrat TRUE
...
** Volání této funkce pri prázdné fronte je vzdy nutné povazovat za nekorektní.
** Bývá to totiy dùsledek spatného návrhu algoritmu, ve kterém je fronta
** pouyita. O takové situaci se proto musí programátor-vývojár dozvedet.
** V opacnem pripade je ladení programù obtiznejsi!
**
** Pøi implementaci vyu¾ijte døíve definované funkce queueEmpty.
*/
	if(queueEmpty(q)) //Testovani zda neni fronta prazdna
	{
		queueError(QERR_FRONT);
		return;
	}

	*c = q->arr[q->f_index]; //Vraceni hodnoty
}

void queueRemove (tQueue* q) {
/*
** Odstraní znak ze zacátku fronty q. Pokud je fronta prázdná, o¹etøete
...
*/
	...
	queueFront(q, c); //Vrati do hodnoty c prvek ze zacatku
	queueRemove(q); //Odstrani prvek ze zacatku fronty
}
...
*/
	if(queueFull(q)) //Testovani zda neni fronta plna
	{
		queueError(QERR_UP);
		return;
	}

	q->arr[q->b_index] = c; //Nacteni hodnoty do fronty
	q->b_index = nextIndex(q->b_index); //Zaplnil jsem index, index prvni volne pozice se musi posunout

	                    /* V pøípadì øe¹ení, sma¾te tento øádek! */
}
