...
**/
	...
	bool found = false; //Zda dany prvek ve strome existuje a nam se ho podarilo najit

	tBTNodePtr TmpPtr = *RootPtr; //Soucasny uzel
	tBTNodePtr PrevTmpPtr; //Otcovsky uzel

	//Vyhledavani ve strome, jestli budeme potom vkladat, nebo hodnota uz existuje 
	if (TmpPtr == NULL)
	{
	 	PrevTmpPtr = NULL;
	 	found = false;
	...
	{
		while(found == false && TmpPtr != NULL)
		{
			//Predchozi prvek je novy otcovsky
			PrevTmpPtr = TmpPtr;

			if(Content < TmpPtr->Cont)
			{
				//Hodnota mensi-> Dale budu hledat v leve vetvi
				TmpPtr = TmpPtr->LPtr;
			}
			else if(Content > TmpPtr->Cont)
			{
				//Hodnota vetsi-> Dale budu hledat v prave vetvi
				TmpPtr = TmpPtr->RPtr;
			}
			else if(Content == TmpPtr->Cont)
			{
				//Nalezl jsem uzel s danou hodnotu
				found = true;
			}
		}
	}

	//Hodnotu jsme nenasli, budeme vkladat
	if(!found)
	{
		tBTNodePtr NewNode;
		...
		if(PrevTmpPtr == NULL)
		{
			//Pokud byl strom prazdny -> vkladame prvni prvek (koren)
			*RootPtr = NewNode;
			(*RootPtr)->Cont = Content;
			(*RootPtr)->LPtr = NULL;
			(*RootPtr)->RPtr = NULL;	
		}
		else if(Content < PrevTmpPtr->Cont)
		{
			//Budeme vkladat do leve vetve (list)
			PrevTmpPtr->LPtr = NewNode;
		...
		{
			//Budeme vkladat do prave vetve (list)
			PrevTmpPtr->RPtr = NewNode;
			NewNode->Cont = Content;
			NewNode->LPtr = NULL;
			NewNode->RPtr = NULL;	
		}
...
**/
	while(ptr != NULL)
	{
		SPushP(Stack, ptr); //Ulozeni uzlu do zasobniku
...
**/
	...
	{
		RootPtr = STopPopP(&stack); //Nacitam prvek ze zasobniku
		if(RootPtr->RPtr != NULL) //Pokud existuje pravy podstrom
		{
			//Zpracuju celou jeho levou vetev
...
**/
	...
	while(ptr != NULL)
	{
		SPushP(Stack, ptr); //Ulozeni ukzatatelu na zasobnik
		ptr = ptr->LPtr;
	}
...
**/
	...
	while(SEmptyP(&stack) != true)
	{
		RootPtr = STopPopP(&stack); //Nactu prvek ze zasobniku
		BTWorkOut(RootPtr); //Zpracuju
		if(RootPtr->RPtr != NULL)
		{
			//Pokud v pravo je nejaky uzel, pokracujeme doprava
...
**/
	...
	tStackP stackP; 
	SInitP(&stackP); //Inicializace zasobniku pointeru

	tStackB stackB;
	SInitB(&stackB); //Inicializace boolean zasobniku

	Leftmost_Postorder(RootPtr, &stackP, &stackB);

	while(SEmptyP(&stackP) != true)
	{
		RootPtr = STopPopP(&stackP); //Nactu prvek ze zasobniku
		zleva = STopPopB(&stackB);

		if(zleva == true) //Prichazi zleva, pujde doprava
		{
		...
		else //Prichazi zprava -> odstrani a zpracuje uzel
		{
			BTWorkOut(RootPtr); //Zpracuju	
		}
	}
...
**/
	...
	while(SEmptyP(&stack) != true && *RootPtr != NULL)
	{

		if(*RootPtr == NULL) //Nacitam uzel ze zasobniku
		{
