...
void initControlPointsDown(S_Vector **points, int offset_x, int offset_y) {
   ...
   */
	...
	*points = vecCreateEmpty(sizeof(Point2d));
	...
	double zmena_zleva = 0;
	double zmena_zprava = 0;

	p.x = 0;   p.y = 0;    point2d_vecPushBack(*points, p); // 0

	p.x = 40;  p.y = -250; point2d_vecPushBack(*points, p); // 1
	p.x = 160; p.y = -250; point2d_vecPushBack(*points, p); // 2

	p.x = 200; p.y = 0;    point2d_vecPushBack(*points, p); // 3

	zmena_zleva = ((200.0 - 160) / (0 + 250)) * 180;
	zmena_zprava = ((390.0 - 360) / (-120 - 0)) * 180;

	// ZDE ZMENA
	p.x = 200 + zmena_zleva; p.y = 180; point2d_vecPushBack(*points, p); // 4
	p.x = 360 + zmena_zprava; p.y = 180; point2d_vecPushBack(*points, p); // 5

	p.x = 360; p.y = 0;    point2d_vecPushBack(*points, p); // 6

	p.x = 390; p.y = -120;  point2d_vecPushBack(*points, p); // 7
	p.x = 430; p.y = -120;  point2d_vecPushBack(*points, p); // 8

	p.x = 460; p.y = 0;    point2d_vecPushBack(*points, p); // 9

	zmena_zleva = ((430.0 - 460) / (-120 - 0)) * 70;
	zmena_zprava = ((545.0 - 535) / (-40 + 0)) * 70;

	// ZDE ZMENA
	p.x = 460 + zmena_zleva; p.y = 70;  point2d_vecPushBack(*points, p); // 10
	p.x = 535 + zmena_zprava; p.y = 70;  point2d_vecPushBack(*points, p); // 11

	p.x = 535; p.y = 0;    point2d_vecPushBack(*points, p); // 12

	p.x = 545; p.y = -40;  point2d_vecPushBack(*points, p); // 13
	p.x = 575; p.y = -40;  point2d_vecPushBack(*points, p); // 14

	p.x = 585; p.y = 0;    point2d_vecPushBack(*points, p); // 15
...
void bezierCubic(const Point2d *P0, const Point2d *P1, const Point2d *P2, const Point2d *P3, 
   ...
   */
	...
	double t;
	double size_of_interval = 1.0;
	double step = size_of_interval / quality;

	for (t = 0.0; t < 1.0; t = t+step)
	{
		Point2d point;

		point.x = P0->x*pow(1 - t, 3) + P1->x*3*t*pow(1 - t, 2) + P2->x*3*pow(t, 2)*(1 - t) + P3->x*pow(t, 3);
		point.y = P0->y*pow(1 - t, 3) + P1->y*3*t*pow(1 - t, 2) + P2->y*3*pow(t, 2)*(1 - t) + P3->y*pow(t, 3);
...
void	bezierCubicsTrajectory(int quality, const S_Vector *control_points, S_Vector *trajectory_points) {
   ...
   * Kostra:
   *  for(...; ...; ...) {
   *    Point2d *P0 = ... vecGetPtr, for point2d_vecSize
   *    Point2d *P1 = ...
   *    Point2d *P2 = ...
   *    Point2d *P3 = ...
