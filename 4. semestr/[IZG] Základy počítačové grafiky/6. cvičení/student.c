...
" vec3 V = normalize(cameraPosition - outputPosition);\n"
" vec3 L = normalize(lightPosition - outputPosition);\n" // Svetlo
" vec3 N = normalize(outputNormal);\n" // Normalovy vektor

" vec3 dM = vec3(0,0.7,0.6);\n"
" vec3 dL = vec3(0,0.7,0.6);\n"
...
void phong_onInit(int32_t width,int32_t height){
  ...
  glCreateBuffers(1, &phong.ebo);
  glCreateVertexArrays(1, &phong.vao);

  // Nahraju data na GPU
  glNamedBufferData(phong.vbo, sizeof(modelVertices), modelVertices, GL_STATIC_DRAW);
  glNamedBufferData(phong.ebo, sizeof(modelIndices), modelIndices, GL_STATIC_DRAW);

  // Nastavi indexy
  glVertexArrayElementBuffer(phong.vao, phong.ebo);

  // stejne pozice jako vertex shaderu
  GLuint position = 0, normal = 1;

  // Nastavi, kde je v bufferu pozice a normala
  glVertexArrayVertexBuffer(phong.vao, position, phong.vbo, 0, (sizeof(GLfloat) * 6));
  glVertexArrayVertexBuffer(phong.vao, normal, phong.vbo, (sizeof(GLfloat) * 3), (sizeof(GLfloat) * 6));
  glEnableVertexArrayAttrib(phong.vao, position);
  glEnableVertexArrayAttrib(phong.vao, normal);
  ...
  phong.viewMatrixUniform = glGetUniformLocation(phong.program, "view");
  phong.projectionMatrixUniform = glGetUniformLocation(phong.program, "projection");
...
void phong_onDraw(){
  ...
  glDrawElements(GL_TRIANGLES, sizeof(modelIndices)/sizeof(int), GL_UNSIGNED_INT, 0);
  ...
  glUniformMatrix4fv(phong.viewMatrixUniform, 1, 0, (GLfloat*)&viewMatrix);
  glUniformMatrix4fv(phong.projectionMatrixUniform, 1, 0, (GLfloat*)&projectionMatrix);
